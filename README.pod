...
This is a Template Toolkit plugin to render L<React|http://facebook.github.io/react/> components. The current version is C<0.002>.

=head1 REQUIREMENTS

Mozilla's L<SpiderMonkey|http://www.mozilla.org/js/spidermonkey/> and L<SWIG|http://www.swig.org>[SWIG] must be installed. SpiderMonkey 24 was used to build this module; if you use a different version, your mileage may vary. The C<Makefile.PL> script uses C<js-config> to learn about the compilation flags to use, so make sure that is in your C<PATH>.
